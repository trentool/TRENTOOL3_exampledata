function analyze_lorenz_2_systems_ensemblemethod

% FUNCTION ANALYZE_LORENZ_2_SYSTEMS_ENSEMBLEMETHOD Analyzes a toy example 
% with two coupled Lorenz systems A1 -> A2 with a delay of 45 ms. This 
% function uses the ensemble method implemented in TRENTOOL (note, this
% requires a NVIDIA GPU for TE estimation). You need to run 'install.m'
% before performing any analysis using GPU functionality.
%
...
addpath('~/TRENTOOL3.3.2')
addpath('~/fieldtrip-lite-20150205');
ft_defaults;

%% define data paths

OutputDataPath = '~/Lorenz_2_results/';
InputDataPath  = '~/TRENTOOL_exampledata/Lorenz_2_systems/';

load([InputDataPath 'lorenz_1-2_45ms.mat']);
...
load([InputDataPath 'lorenz_layout.mat']);
